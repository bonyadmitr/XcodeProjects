...
open class CameraManager3rdParty: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate {
    ...
    fileprivate func attachZoom(_ view: UIView) {
        zoomGesture.addTarget(self, action: #selector(CameraManager3rdParty._zoomStart(_:)))
    ...
    fileprivate func attachFocus(_ view: UIView) {
        focusGesture.addTarget(self, action: #selector(CameraManager3rdParty._focusStart(_:)))
    ...
    fileprivate func _startFollowingDeviceOrientation() {
        if shouldRespondToOrientationChanges && !cameraIsObservingDeviceOrientation {
            NotificationCenter.default.addObserver(self, selector: #selector(CameraManager3rdParty._orientationChanged), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
    ...
    open func _doFlipAnimation() {
        ...
        if let validEmbeddingView = embeddingView {
            if let validPreviewLayer = previewLayer {
                ...
                if CameraManager3rdParty._blurSupported() {
