...
extension SystemUtils {
    ...
    private static var lockedDeviceURL: URL? {
        let directoryURL = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.by.TodayExtensionTest111q")
    ...
    public static func isDeviceLocked() -> Bool {
        ...
        do {
            let q = try Data(contentsOf: lockFileURL, options: .mappedIfSafe)
            print(String(data: q, encoding: .utf8)!)
            return false
